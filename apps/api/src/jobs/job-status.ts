...
export function computeJobStatus(
  ...
  const remainingDays = Math.ceil((endDate.getTime() - now.getTime()) / DAY_MS);
  if (remainingDays <= RENTAL_CRITICAL_DAYS) return JobStatus.CRITICAL;
  if (remainingDays <= RENTAL_WARNING_DAYS) return JobStatus.WARNING;
