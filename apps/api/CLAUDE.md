...
- Mold tracking sebagian besar otomatis: PLANNING, DELIVERY, RECEIVED, dan PRODUCTION digerakkan event domain lewat `MoldTrackingService.advance` (idempoten, hanya maju) di dalam transaksi service pemicu. Hanya SEND_BACK dan COMPLETED lewat `PATCH /molds/:id/tracking`, dan pemilik tombolnya berbeda: SEND_BACK milik ADMIN_SUNDAYA, COMPLETED milik MANAGER_PENYEWA pemilik cetakan (approval pengembalian, per cetakan bukan per job). Peta pemiliknya di `MOLD_MANUAL_TRANSITIONS` (shared). Lihat PROJECT_CONTEXT.md bagian 5a.
- Lifecycle job juga sebagian otomatis. Tidak ada endpoint kirim/aktifkan/selesaikan job: mesin tidak pernah keluar dari Sundaya. `DIKONFIRMASI -> AKTIF` dipicu Log Penerimaan cetakan, `AKTIF -> SELESAI` dipicu cetakan terakhir booking yang jadi COMPLETED. Keduanya fungsi lepas bertx di `jobs/job-transitions.ts`, dipanggil dari dalam transaksi service pemicu dan idempoten seperti `advance`. Jangan menambahkan tombol untuk keduanya.
- Log Produksi hanya dua jenis event (PRODUKSI_HARIAN, PROGRESS_MOLDING) dan `machineId` selalu wajib. Kedatangan material hidup di Log Pengiriman dan Log Penerimaan; jangan mengembalikan event material datang ke Layer 2.
