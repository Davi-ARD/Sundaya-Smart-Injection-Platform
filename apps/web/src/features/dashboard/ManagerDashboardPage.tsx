  ...
  return (
    <div className="mx-auto max-w-screen-2xl">
      <PageHeader
        ...
        title="Dashboard"
        description={`Ringkasan produksi dan pengiriman ${user?.companyName ?? 'perusahaan Anda'}.`}
      />
        ...
        {isLoading ? (
          ...
          <div className="grid place-items-center py-12 text-center">
            <span className="grid h-12 w-12 place-items-center text-brand-700">
              {molds.length === 0 ? <Boxes className="h-7 w-7" /> : <CalendarPlus className="h-7 w-7" />}
            </span>
            ...
            >
              {molds.length === 0 ? 'Tambah cetakan' : 'Ajukan booking'} <ArrowRight className="h-5 w-5" />
            </Link>
          </div>
        ) : (
